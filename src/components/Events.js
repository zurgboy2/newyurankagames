...
const EventsSection = () => {
  ...
  const weeklyEventCounts = filters.reduce((acc, filter) => {
    if (filter.key === "all") {
      ...
      acc[filter.key] = weeklyEvents.filter(
        (t) => t.eventType === filter.key
      ).length;
    }
    return acc;
  }, {});
  ...
  useEffect(() => {
    setShowAllWeekly(false);
  }, [selectedWeeklyFilter, tournaments]);
  const displayedWeeklyEvents = showAllWeekly
  ...
  const monthlyEventCounts = filters.reduce((acc, filter) => {
    if (filter.key === "all") {
      ...
      acc[filter.key] = monthlyEvents.filter(
        (t) => t.eventType === filter.key
      ).length;
    }
    return acc;
  ...
  }, [selectedMonthlyFilter, tournaments]);
  const displayedMonthlyEvents = showAllMonthly
    ? filteredMonthlyEvents
    : filteredMonthlyEvents.slice(0, 8);
  ...
  return (
    <div className="events-container">
            ...
            <div key={index} className="tournament-card">
              ...
              <div className="card-details">
                ...
                <div className="detail-item">
                  ...
                  <span className="detail-value">
                    {formatTime(tournament.date) || "To be informed"}
                  </span>
                </div>
                <div className="detail-item">
                  <span className="label">Entrance fee:</span>
                  <span className="detail-value">
                    {tournament.price + " EUR" || "To be informed"}
                  </span>
                </div>
                {tournament.prizes && (
                  <div className="detail-item">
      ...
      <h2 className="events-subtitle" ref={monthlyTournamentsRef}>
        Monthly Events
      </h2>
        ...
        {filters.map((filter) => (
          <button
            ...
            className={`filter-button ${
              selectedMonthlyFilter === filter.key ? "active" : ""
            }`}
          ...
          <div className="tournament-cards-container">
            ...
            {displayedMonthlyEvents.map((tournament, index) => (
              <div key={index} className="tournament-card">
                ...
                <div className="card-details">
                  <div
                    className="card-details-scroll"
                    dangerouslySetInnerHTML={{
                      __html: tournament.descriptionHtml,
                    }}
                  />
                  <div className="detail-item">
                    <span className="detail-readerinfo">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (!newState && monthlyTournamentsRef.current) {
                    monthlyTournamentsRef.current.scrollIntoView({
                      behavior: "smooth",
                      block: "start",
                    });
                    setTimeout(() => {
      ...
      <h2 className="events-subtitle" ref={weeklyTournamentsRef}>
        Weekly Events
      </h2>
      ...
      <div className="filters-container">
        {filters.map((filter) => (
          <button
            key={filter.key}
            className={`filter-button ${
              selectedWeeklyFilter === filter.key ? "active" : ""
            }`}
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (!newState && weeklyTournamentsRef.current) {
                    weeklyTournamentsRef.current.scrollIntoView({
                      behavior: "smooth",
                      block: "start",
                    });
                    setTimeout(() => {
...
const TournamentDetailsSection = ({ tournament }) => {
  ...
  async function handleRegistration(e) {
    ...
    try {
      const result = await makeRegistrationRequestCall(
        "tournament_script",
        "registerForEvent",
        {
  ...
  return (
    <div className="tournament-details-section">
      ...
      <div className="details-top">
        ...
        <div className="details-column">
          ...
          <div className="detail-row">
            ...
            <span className="detail-val">
              {formatTime(tournament.date) || "To be informed. Stay tuned!"}
            </span>
          </div>
          ...
          {tournament.description && (
            <div className="detail-row">
              ...
              <span className="detail-value-bundle">
                {tournament.description}
              </span>
            </div>
          )}
          {tournament.preReleaseBundleIncludes && (
            <div className="detail-row">
              <span className="detail-label-bundle">
                Pre-Release Bundle Includes:
              </span>
              <span className="detail-value-bundle">
                {tournament.preReleaseBundleIncludes}
          ...
          {tournament.rules && (
            <div className="detail-row">
              ...
              <span className="detail-value-rules">{tournament.rules}</span>
            </div>
          )}
          {tournament.price && (
            <div className="detail-row">
              ...
              <span className="detail-val">
                {tournament.price || "To be informed. Stay tuned!"} EUR
              </span>
            </div>
          )}
        </div>
      ...
      <div className="disclaimer-text">
        Your personal information is collected solely for the purpose of
        tournament organization and communication. All data will be securely
        stored and deleted within 30 days after the tournament's conclusion.
      </div>
      <div className="disclaimer-text">
        For your convenience, secure online payment options are available.
        Follow the provided checkout link to pay by card, or choose to pay in
        person at our store by cash.
      </div>
      <div className="disclaimer-text">
        Ticket Purchase Policy: Tickets are non-refundable. If you cannot attend
        the registered tournament, your ticket will be valid for the next event
        in the series. Should you miss all events, collect your boosters the
        following business day at our store.
      </div>

      <button className="proceed-button" onClick={handleRegistration}>
        Register
      </button>
